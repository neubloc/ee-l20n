...
define('l20n/html', function(require) {
  ...
  function replaceDataAttributes(value, node) {
    return value.replace(/\[\[(.+?)\]\]/ig, function(match, attribute) {
      return node.dataset[attribute] || 'undefined attribute ' + attribute
    })
  }
  ...
  function translateNode(node, id, entity) {
    ...
    if (entity.value) {
      ...
      if (entity.value.indexOf('<') === -1 &&
        ...
        node.textContent = replaceDataAttributes(entity.value, node);
      } else {
    ...
    Object.keys(entity.attributes).forEach(function(key) {
      ...
      if (isAttrAllowed({ name: attrName }, node)) {
        node.setAttribute(attrName, replaceDataAttributes(entity.attributes[key], node));
      }
    });
  }
